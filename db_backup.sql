...
/*!50003 SET collation_connection  = utf8mb4_general_ci */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_candy`(n_input VARCHAR(100), iu_input VARCHAR(2048), desc_input VARCHAR(250))
    MODIFIES SQL DATA
BEGIN
	insert into candy (name, image_url, description)
	values (n_input, iu_input, desc_input);
...
BEGIN
	select id, name, image_url, description from candy
	order by id desc;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
